...
namespace Serveri_UDP
{
    class Program
    {
        private static Socket sFillimi;
        public static string sDhena;
        public static string sDhena1;
        private static string PortNumber;
        private static EndPoint endp;
        static void Main(string[] args)
        {
            Console.Write("Serveri FIEK-UDP");

            IPEndPoint ip = new IPEndPoint(IPAddress.Any, 7000);
            Socket sFillimi = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            sFillimi.Bind(ip);
            while (true)
            {
                 endp = new IPEndPoint(IPAddress.Any, 7000);
                Console.WriteLine("\nServeri startoi dhe po pret per t'u lidhur me ndonje klient...");   
                byte[] bEdhena = new byte[128];
                int gjatesia = sFillimi.ReceiveFrom(bEdhena, ref endp);
                sDhena = Encoding.ASCII.GetString(bEdhena, 0, gjatesia);

                Console.WriteLine("\nServeri edhte lidhur me Klientin ");
                if (sDhena == "0")
                {
                ...
                else if (sDhena == "1")
                {
                    ...
                    Pergjigjja = Encoding.ASCII.GetBytes(sDhena1);
                    sFillimi.SendTo(Pergjigjja, endp);
                }
                ...
                byte[] bEdhenaPerfundimtare = Encoding.ASCII.GetBytes(sDhena1);
                sFillimi.SendTo(bEdhenaPerfundimtare, endp);
        ...
        {
            string klientIP = endp.ToString();
            int Ip_port = klientIP.LastIndexOf(":") + 1;
            PortNumber = klientIP.Substring(Ip_port);
            return PortNumber;
        }
